...
import stage3_3_mode, stage3_9_mode
import common
        ...
        if event.type == SDL_QUIT:
            ...
            if 500 <= common.player.x <=  550 and 580 <= common.player.y <= 600: # 상단 문 (메인 던전으로 가는 문)
                if not Stage3_3.stage3_3_create:
                    game_framework.push_mode(stage3_3_mode,(525, 0))
                else:
                    game_framework.pop_mode(stage3_3_mode,(525, 0))
            elif 500 <= common.player.x <=  550 and 0 <= common.player.y <= 20: # 하단 문
                if not Stage3_9.stage3_9_create:
                    game_framework.push_mode(stage3_9_mode,(525, 600))
                else:
                    game_framework.pop_mode(stage3_9_mode,(525, 600))
        else:
...
def init(player_start_pos=None):
    ...
    if not Stage3_6.stage3_6_create:
        Stage3_6.stage3_6_create = True
        Stage3_6.current_mode = True

        goblin_mobs = [Goblin_Mob() for _ in range(random.randint(2, 5))]
        for goblin_mob in goblin_mobs:
            goblin_mob.move_validator = stage3_6.is_mob_walkable
    ...
    game_world.add_object(stage3_6, 0)

    common.player = Player()
    common.player.move_validator = stage3_6.is_walkable
    if player_start_pos:
...
def pause():
    global goblin_mobs, stage3_6, coins
    global ui

    Stage3_6.current_mode = False
    ...
    for goblin_mob in goblin_mobs:
        if goblin_mob.is_alive:
            ...
            stage3_6.saved_mobs.append(mob_data)
            # print(f"Pause: Saved mob at ({mob_data['x']}, {mob_data['y']}) with type '{mob_data['type']}', HP: {mob_data['hp']}")

    # 코인 저장
    stage3_6.saved_coins = []
    for coin in coins:
        stage3_6.saved_coins.append({
            'x': coin.x,
            'y': coin.y,
            'frame': coin.frame
        })

    # print(f"Pause: Saved {len(stage3_6.saved_mobs)} zombie mobs, {len(stage3_6.saved_coins)} coins")
...
def resume(player_start_pos=None):
    global goblin_mobs, stage3_6, player, coins
    global ui
    ...
    # 저장된 몹 복원
    if stage3_6.saved_mobs:
        goblin_mobs = []
        for mob_data in stage3_6.saved_mobs:
    ...
    # 코인 복원
    if stage3_6.saved_coins:
        coins = []
        for coin_data in stage3_6.saved_coins:
