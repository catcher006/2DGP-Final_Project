...
import stage2_2_mode, stage2_4_mode, stage2_8_mode
import common
        ...
        if event.type == SDL_QUIT:
            ...
            if 500 <= common.player.x <= 550 and 580 <= common.player.y <= 600:  # 상단 문
                if not Stage2_2.stage2_2_create:
                    game_framework.push_mode(stage2_2_mode, (525, 0))
                else:
                    game_framework.pop_mode(stage2_2_mode, (525, 0))
            elif 500 <= common.player.x <=  550 and 0 <= common.player.y <= 20: # 하단 문
                if not Stage2_8.stage2_8_create:
                    game_framework.push_mode(stage2_8_mode,(525, 600))
                else:
                    game_framework.pop_mode(stage2_8_mode,(525, 600))
            elif 50 <= common.player.x <= 70 and 270 <= common.player.y <= 370:  # 좌측 문
                if not Stage2_4.stage2_4_create:
                    game_framework.push_mode(stage2_4_mode, (1010, 320))
                else:
                    game_framework.pop_mode(stage2_4_mode, (1010, 320))
        else:
...
def init(player_start_pos=None):
    ...
    game_world.add_object(stage2_5, 0)

    common.player = Player()
    common.player.move_validator = stage2_5.is_walkable
    if player_start_pos:
...
def pause():
    ...
    for zombie_mob in zombie_mobs:
        if zombie_mob.is_alive:
            ...
            stage2_5.saved_mobs.append(mob_data)
            # print(f"Pause: Saved mob at ({mob_data['x']}, {mob_data['y']}) with type '{mob_data['type']}', HP: {mob_data['hp']}")

    # 코인 저장
    stage2_5.saved_coins = []
    for coin in coins:
        stage2_5.saved_coins.append({
            'x': coin.x,
            'y': coin.y,
            'frame': coin.frame
        })

    # print(f"Pause: Saved {len(stage2_5.saved_mobs)} zombie mobs, {len(stage2_5.saved_coins)} coins")
...
def resume(player_start_pos=None):
    ...
    # 저장된 몹 복원
    if stage2_5.saved_mobs:
        zombie_mobs = []
        for mob_data in stage2_5.saved_mobs:
    ...
    # 코인 복원
    if stage2_5.saved_coins:
        coins = []
        for coin_data in stage2_5.saved_coins:
