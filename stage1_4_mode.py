...
def handle_events():
    ...
    for event in event_list:
        if event.type == SDL_QUIT:
            ...
            stage1_manger.stage1_4_mobs = mobs_state

            if 990 <= player.x <=  1010 and 270 <= player.y <= 370: # 우측 문
                game_framework.change_mode(stage1_5_mode, (50, 320))
            elif 500 <= player.x <=  550 and 0 <= player.y <= 20: # 하단 문
                game_framework.change_mode(stage1_7_mode, (525, 600))
        else:
...
def init(player_start_pos=None):
    ...
    slime_mobs = []
    if not stage1_manger.stage1_4_create:
        ...
        # 슬라임이 0개여도 생성 완료로 표시
        stage1_manger.stage1_4_create = True

        # 초기 상태를 빈 리스트로 저장 (0개인 경우 대비)
        if not slime_mobs:
            stage1_manger.stage1_4_mobs = []
    else:
        # 저장된 상태에서 복원
        saved_mobs = stage1_manger.stage1_4_mobs  # 함수가 아닌 변수 직접 사용
        if saved_mobs:
            for mob_state in saved_mobs:
                ...
                slime.move_validator = stage1_4.is_mob_walkable
