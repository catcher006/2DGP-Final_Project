...
import stage3_5_mode, stage3_11_mode
import common
        ...
        if event.type == SDL_QUIT:
            ...
            if 500 <= common.player.x <=  550 and 580 <= common.player.y <= 600: # 상단 문 (메인 던전으로 가는 문)
                if not Stage3_5.stage3_5_create:
                    game_framework.push_mode(stage3_5_mode,(525, 0))
                else:
                    game_framework.pop_mode(stage3_5_mode,(525, 0))
            elif 500 <= common.player.x <=  550 and 0 <= common.player.y <= 20: # 하단 문
                if not Stage3_11.stage3_11_create:
                    game_framework.push_mode(stage3_11_mode,(525, 600))
                else:
                    game_framework.pop_mode(stage3_11_mode,(525, 600))
        else:
...
def init(player_start_pos=None):
    ...
    if not Stage3_8.stage3_8_create:
        Stage3_8.stage3_8_create = True
        Stage3_8.current_mode = True

        goblin_mobs = [Goblin_Mob() for _ in range(random.randint(2, 5))]
        for goblin_mob in goblin_mobs:
            goblin_mob.move_validator = stage3_8.is_mob_walkable
    ...
    game_world.add_object(stage3_8, 0)

    common.player = Player()
    common.player.move_validator = stage3_8.is_walkable
    if player_start_pos:
...
def pause():
    global goblin_mobs, stage3_8, coins
    global ui

    Stage3_8.current_mode = False
...
def resume(player_start_pos=None):
    global goblin_mobs, stage3_8, player, coins
    global ui
    ...
    if stage3_8.saved_coins:
        coins = []
        for coin_data in stage3_8.saved_coins:
