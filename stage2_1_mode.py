        ...
        # 마우스 이벤트는 무시
            ...
            if 990 <= common.player.x <=  1010 and 270 <= common.player.y <= 370: # 우측 문
                if not Stage2_2.stage2_2_create:
                    game_framework.push_mode(stage2_2_mode,(50, 320))
                else:
                    game_framework.pop_mode(stage2_2_mode,(50, 320))
...
def init(player_start_pos=None):
    ...
    if not Stage2_1.stage2_1_create:
        Stage2_1.stage2_1_create = True
        Stage2_1.current_mode = True

        zombie_mobs = [Zombie_Mob() for _ in range(random.randint(2, 5))]
        for zombie_mob in zombie_mobs:
            zombie_mob.move_validator = stage2_1.is_mob_walkable
    ...
    game_world.add_object(stage2_1, 0)

    common.player = Player()
    common.player.move_validator = stage2_1.is_walkable
    if player_start_pos:
...
def pause():
    global zombie_mobs, stage2_1, coins

    Stage2_1.current_mode = False
    ...
    for zombie_mob in zombie_mobs:
        if zombie_mob.is_alive:
            ...
            stage2_1.saved_mobs.append(mob_data)
            print(f"Pause: Saved mob at ({mob_data['x']}, {mob_data['y']}) with type '{mob_data['type']}', HP: {mob_data['hp']}")

    # 코인 저장
    stage2_1.saved_coins = []
    for coin in coins:
        stage2_1.saved_coins.append({
            'x': coin.x,
            'y': coin.y,
            'frame': coin.frame
        })

    print(f"Pause: Saved {len(stage2_1.saved_mobs)} zombie mobs, {len(stage2_1.saved_coins)} coins")
...
def resume(player_start_pos=None):
    global zombie_mobs, stage2_1, player, coins
    ...
    # 저장된 몹 복원
    if stage2_1.saved_mobs:
        zombie_mobs = []
        for mob_data in stage2_1.saved_mobs:
    ...
    # 코인 복원
    if stage2_1.saved_coins:
        coins = []
        for coin_data in stage2_1.saved_coins:
