...
import stage3_9_mode, stage3_11_mode
import common
        ...
        if event.type == SDL_QUIT:
            ...
            if 990 <= common.player.x <=  1010 and 270 <= common.player.y <= 370: # 우측 문
                if not Stage3_11.stage3_11_create:
                    game_framework.push_mode(stage3_11_mode,(50, 320))
                else:
                    game_framework.pop_mode(stage3_11_mode,(50, 320))
            elif 50 <= common.player.x <= 70 and 270 <= common.player.y <= 370:  # 좌측 문
                if not Stage3_9.stage3_9_create:
                    game_framework.push_mode(stage3_9_mode, (1010, 320))
                else:
                    game_framework.pop_mode(stage3_9_mode, (1010, 320))
        else:
...
def init(player_start_pos=None):
    ...
    if not Stage3_10.stage3_10_create:
        Stage3_10.stage3_10_create = True
        Stage3_10.current_mode = True

        goblin_mobs = [Goblin_Mob() for _ in range(random.randint(2, 5))]
        for goblin_mob in goblin_mobs:
            goblin_mob.move_validator = stage3_10.is_mob_walkable
    ...
    game_world.add_object(stage3_10, 0)

    common.player = Player()
    common.player.move_validator = stage3_10.is_walkable
    if player_start_pos:
...
def pause():
    global goblin_mobs, stage3_10, coins

    Stage3_10.current_mode = False

    # 기존 goblin_mobs 초기화 후 현재 살아있는 몹만 저장
    stage3_10.saved_mobs = []
    for goblin_mob in goblin_mobs:
        if goblin_mob.is_alive:
            ...
            stage3_10.saved_mobs.append(mob_data)
            print(f"Pause: Saved mob at ({mob_data['x']}, {mob_data['y']}) with type '{mob_data['type']}', HP: {mob_data['hp']}")

    # 코인 저장
    stage3_10.saved_coins = []
    for coin in coins:
        stage3_10.saved_coins.append({
            'x': coin.x,
            'y': coin.y,
            'frame': coin.frame
        })

    print(f"Pause: Saved {len(stage3_10.saved_mobs)} zombie mobs, {len(stage3_10.saved_coins)} coins")
...
def resume(player_start_pos=None):
    global goblin_mobs, stage3_10, player, coins
    ...
    # 저장된 몹 복원
    if stage3_10.saved_mobs:
        goblin_mobs = []
        for mob_data in stage3_10.saved_mobs:
    ...
    # 코인 복원
    if stage3_10.saved_coins:
        coins = []
        for coin_data in stage3_10.saved_coins:
