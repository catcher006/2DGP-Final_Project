...
import stage2_1_mode, stage2_3_mode, stage2_5_mode
import common
        ...
        if event.type == SDL_QUIT:
            ...
            if 500 <= common.player.x <= 550 and 580 <= common.player.y <= 600:  # 상단 문
                if not Stage2_1.stage2_1_create:
                    game_framework.push_mode(stage2_1_mode, (525, 0))
                else:
                    game_framework.pop_mode(stage2_1_mode, (525, 0))
            elif 990 <= common.player.x <= 1010 and 270 <= common.player.y <= 370:  # 우측 문
                if not Stage2_5.stage2_5_create:
                    game_framework.push_mode(stage2_5_mode, (50, 320))
                else:
                    game_framework.pop_mode(stage2_5_mode, (50, 320))
            elif 50 <= common.player.x <= 70 and 270 <= common.player.y <= 370:  # 좌측 문
                if not Stage2_3.stage2_3_create:
                    game_framework.push_mode(stage2_3_mode, (1010, 320))
                else:
                    game_framework.pop_mode(stage2_3_mode, (1010, 320))
        else:
...
def init(player_start_pos=None):
    ...
    if not Stage2_4.stage2_4_create:
        Stage2_4.stage2_4_create = True
        Stage2_4.current_mode = True

        zombie_mobs = [Zombie_Mob() for _ in range(random.randint(2, 5))]
        for zombie_mob in zombie_mobs:
            zombie_mob.move_validator = stage2_4.is_mob_walkable
    ...
    game_world.add_object(stage2_4, 0)

    common.player = Player()
    common.player.move_validator = stage2_4.is_walkable
    if player_start_pos:
    ...
    if zombie_mobs:
        ...
        game_world.add_collision_pair('player:zombie_mob', common.player, None)
        for zombie_mob in zombie_mobs:
...
def pause():
    global zombie_mobs, stage2_4, coins
    global ui

    Stage2_4.current_mode = False
    ...
    for zombie_mob in zombie_mobs:
        if zombie_mob.is_alive:
            ...
            stage2_4.saved_mobs.append(mob_data)
            # print(f"Pause: Saved mob at ({mob_data['x']}, {mob_data['y']}) with type '{mob_data['type']}', HP: {mob_data['hp']}")

    # 코인 저장
    stage2_4.saved_coins = []
    for coin in coins:
        stage2_4.saved_coins.append({
            'x': coin.x,
            'y': coin.y,
            'frame': coin.frame
        })

    # print(f"Pause: Saved {len(stage2_4.saved_mobs)} zombie mobs, {len(stage2_4.saved_coins)} coins")
...
def resume(player_start_pos=None):
    global zombie_mobs, stage2_4, player, coins
    global ui
    ...
    # 저장된 몹 복원
    if stage2_4.saved_mobs:
        zombie_mobs = []
        for mob_data in stage2_4.saved_mobs:
    ...
    # 코인 복원
    if stage2_4.saved_coins:
        coins = []
        for coin_data in stage2_4.saved_coins:
