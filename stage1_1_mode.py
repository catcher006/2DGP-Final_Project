        ...
        if event.type == SDL_QUIT:
            ...
            if 990 <= common.player.x <=  1010 and 270 <= common.player.y <= 370: # 우측 문
                if not Stage1_2.stage1_2_create:
                    game_framework.push_mode(stage1_2_mode, (50, 320))
                else:
                    game_framework.pop_mode(stage1_2_mode, (50, 320))
            elif 50 <= common.player.x <= 70 and 270 <= common.player.y <= 370:  # 좌측 문
                if not Stage1_0.stage1_0_create:
                    game_framework.push_mode(stage1_0_mode, (1010, 320))
                else:
                    game_framework.pop_mode(stage1_0_mode, (1010, 320))
        else:
...
def init(player_start_pos=None):
    ...
    if not Stage1_1.stage1_1_create:
        Stage1_1.stage1_1_create = True
        Stage1_1.current_mode = True
    ...
    common.player.move_validator = stage1_1.is_walkable
    if player_start_pos:
...
def pause():
    global slime_mobs, stage1_1, coins
    global ui

    Stage1_1.current_mode = False

    # 기존 saved_mobs 초기화 후 현재 살아있는 몹만 저장
    stage1_1.saved_mobs = []
    for slime_mob in slime_mobs:
        if slime_mob.is_alive:
            stage1_1.saved_mobs.append({
    ...
    # 코인 저장
    stage1_1.saved_coins = []
    for coin in coins:
        stage1_1.saved_coins.append({
            'x': coin.x,
            'y': coin.y,
            'frame': coin.frame
        })

    # print(f"Pause: Saved {len(stage1_1.saved_mobs)} slime mobs, {len(stage1_1.saved_coins)} coins")
...
def resume(player_start_pos=None):
    global slime_mobs, stage1_1, player, coins
    global ui
    ...
    # stage1_1 인스턴스에 저장된 몹 복원
    if stage1_1.saved_mobs:
        slime_mobs = []
        for mob_data in stage1_1.saved_mobs:
            slime_mob = Slime_Mob()
    ...
    # 코인 복원
    if stage1_1.saved_coins:
        coins = []
        for coin_data in stage1_1.saved_coins:
            coin = Coin()
            coin.x = coin_data['x']
            coin.y = coin_data['y']
