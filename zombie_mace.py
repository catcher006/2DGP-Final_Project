...
class Zombie_Mace():
    ...
    def update(self):
        # 좀비 객체에서 직접 값을 가져옴
        self.x = self.zombie.x
        self.y = self.zombie.y
        self.frame = int(self.zombie.frame)
        self.face_dir = int(self.zombie.face_dir)

        # 공격 프레임이 끝나거나 좀비가 공격 중이 아니거나 죽었으면 제거
        is_attacking = self.zombie.is_attacking
        is_alive = self.zombie.is_alive
