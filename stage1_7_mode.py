...
def handle_events():
    ...
    for event in event_list:
        if event.type == SDL_QUIT:
            ...
            stage1_manger.stage1_7_mobs = mobs_state

            if 500 <= player.x <=  550 and 580 <= player.y <= 600: # 상단 문
                game_framework.change_mode(stage1_4_mode, (525, 0))
            elif 50 <= player.x <= 70 and 270 <= player.y <= 370:  # 좌측 문
                game_framework.change_mode(stage1_6_mode, (1010, 320))
        else:
...
def init(player_start_pos=None):
    ...
    slime_mobs = []
    if not stage1_manger.stage1_7_create:
        ...
        # 슬라임이 0개여도 생성 완료로 표시
        stage1_manger.stage1_7_create = True

        # 초기 상태를 빈 리스트로 저장 (0개인 경우 대비)
        if not slime_mobs:
            stage1_manger.stage1_7_mobs = []
    else:
        # 저장된 상태에서 복원
        saved_mobs = stage1_manger.stage1_7_mobs  # 함수가 아닌 변수 직접 사용
        if saved_mobs:
            for mob_state in saved_mobs:
                ...
                slime.move_validator = stage1_7.is_mob_walkable
