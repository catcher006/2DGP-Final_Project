...
class Attack:
    ...
    def enter(self, e):
        ...
        if Stage3_7.current_mode:
            game_world.add_collision_pair('player:goblin_boss_sword', None, self.goblin_boss_sword)
            # 플레이어와 충돌 페어 추가
...
class Goblin_Boss:
    def __init__(self):
        ...
        self.dead_image = load_image("./image/mobs/goblin_boss/dead.png")
        self.attack_image = load_image("./image/mobs/goblin_boss/attack.png")
    ...
    def draw(self):
        ...
        if self.is_alive or self.hp > 0:
            self.hp_image.clip_draw(0, int(self.hp // 3) * 54, 400, 54, self.x, self.y + 35, 60, 11)
            if self.hp >= 100:
    ...
    def handle_collision(self, group, other):
        ...
        if group == 'player_sword:goblin_boss' and self.is_alive:
            ...
            if current_time - self.last_damage_time >= self.damage_cooldown:
                ...
                # 디버그 출력 (선택사항)
                print(f"goblin_boss damaged! HP: {self.hp}")
