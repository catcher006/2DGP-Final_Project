        ...
        if event.type == SDL_QUIT:
            ...
            if 500 <= common.player.x <=  550 and 580 <= common.player.y <= 600: # 상단 문
                if not Stage1_3.stage1_3_create:
                    game_framework.push_mode(stage1_3_mode,(525, 0))
                else:
                    game_framework.pop_mode(stage1_3_mode, (525, 0))
            elif 990 <= common.player.x <=  1010 and 270 <= common.player.y <= 370: # 우측 문
                if not Stage1_7.stage1_7_create:
                    game_framework.push_mode(stage1_7_mode, (50, 320))
                else:
                    game_framework.pop_mode(stage1_7_mode, (50, 320))
        else:
...
def init(player_start_pos=None):
    ...
    if not Stage1_6.stage1_6_create:
        Stage1_6.stage1_6_create = True
        Stage1_6.current_mode = True

        slime_mobs = [Slime_Mob() for _ in range(random.randint(2, 4))]
        for slime_mob in slime_mobs:
            slime_mob.move_validator = stage1_6.is_mob_walkable
    ...
    game_world.add_object(stage1_6, 0)

    common.player = Player()
    common.player.move_validator = stage1_6.is_walkable
    # 시작 좌표 설정
    if player_start_pos:
...
def pause():
    global slime_mobs, stage1_6, coins
    global ui

    Stage1_6.current_mode = False

    # 기존 saved_mobs 초기화 후 현재 살아있는 몹만 저장
    stage1_6.saved_mobs = []
    for slime_mob in slime_mobs:
        if slime_mob.is_alive:
            stage1_6.saved_mobs.append({
    ...
    # 코인 저장
    stage1_6.saved_coins = []
    for coin in coins:
        stage1_6.saved_coins.append({
            'x': coin.x,
            'y': coin.y,
            'frame': coin.frame
        })

    # print(f"Pause: Saved {len(stage1_6.saved_mobs)} slime mobs, {len(stage1_6.saved_coins)} coins")
...
def resume(player_start_pos=None):
    global slime_mobs, stage1_6, player, coins
    global ui
    ...
    # stage1_6 인스턴스에 저장된 몹 복원
    if stage1_6.saved_mobs:
        slime_mobs = []
        for mob_data in stage1_6.saved_mobs:
            slime_mob = Slime_Mob()
    ...
    # 코인 복원
    if stage1_6.saved_coins:
        coins = []
        for coin_data in stage1_6.saved_coins:
            coin = Coin()
            coin.x = coin_data['x']
            coin.y = coin_data['y']
