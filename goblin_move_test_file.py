...
class Goblin_Mob:
    def __init__(self):
        self.walk_image = load_image('goblin_bow_walk.png')
    ...
    for event in events:
        if event.type == SDL_QUIT:
            ...
            if event.key == SDLK_ESCAPE:
                ...
                goblin_mob.set_direction(0, 5, 'up')
            elif event.key == SDLK_s:
                goblin_mob.set_direction(0, -5, 'down')
            elif event.key == SDLK_a:
                goblin_mob.set_direction(-5, 0, 'left')
            elif event.key == SDLK_d:
                goblin_mob.set_direction(5, 0, 'right')
...
def reset_world():
    ...
    goblin_mob = Goblin_Mob()  # skeleton_mob 객체 생성
    world.append(goblin_mob)  # 월드에 추가
