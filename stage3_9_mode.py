...
import stage3_6_mode, stage3_10_mode
import common
        ...
        if event.type == SDL_QUIT:
            ...
            if 500 <= common.player.x <=  550 and 580 <= common.player.y <= 600: # 상단 문 (메인 던전으로 가는 문)
                if not Stage3_6.stage3_6_create:
                    game_framework.push_mode(stage3_6_mode, (525, 0))
                else:
                    game_framework.pop_mode(stage3_6_mode, (525, 0))
            elif 990 <= common.player.x <=  1010 and 270 <= common.player.y <= 370: # 우측 문
                if not Stage3_10.stage3_10_create:
                    game_framework.push_mode(stage3_10_mode,(50, 320))
                else:
                    game_framework.pop_mode(stage3_10_mode,(50, 320))
        else:
...
def init(player_start_pos=None):
    ...
    if not Stage3_9.stage3_9_create:
        Stage3_9.stage3_9_create = True
        Stage3_9.current_mode = True

        goblin_mobs = [Goblin_Mob() for _ in range(random.randint(2, 5))]
        for goblin_mob in goblin_mobs:
            goblin_mob.move_validator = stage3_9.is_mob_walkable
    ...
    game_world.add_object(stage3_9, 0)

    common.player = Player()
    common.player.move_validator = stage3_9.is_walkable
    if player_start_pos:
...
def pause():
    global goblin_mobs, stage3_9, coins
    global ui

    Stage3_9.current_mode = False
    ...
    for goblin_mob in goblin_mobs:
        if goblin_mob.is_alive:
            ...
            stage3_9.saved_mobs.append(mob_data)
            print(f"Pause: Saved mob at ({mob_data['x']}, {mob_data['y']}) with type '{mob_data['type']}', HP: {mob_data['hp']}")

    # 코인 저장
    stage3_9.saved_coins = []
    for coin in coins:
        stage3_9.saved_coins.append({
            'x': coin.x,
            'y': coin.y,
            'frame': coin.frame
        })

    print(f"Pause: Saved {len(stage3_9.saved_mobs)} zombie mobs, {len(stage3_9.saved_coins)} coins")
...
def resume(player_start_pos=None):
    global goblin_mobs, stage3_9, player, coins
    global ui
    ...
    # 저장된 몹 복원
    if stage3_9.saved_mobs:
        goblin_mobs = []
        for mob_data in stage3_9.saved_mobs:
    ...
    # 코인 복원
    if stage3_9.saved_coins:
        coins = []
        for coin_data in stage3_9.saved_coins:
