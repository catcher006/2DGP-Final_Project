        ...
        if self.played:
            ...
            if not self.spawned:
                ...
                if Stage3_0.current_mode:
                    ...
                    stage3_3_mode.coins.append(coin)
                    ...
                    stage3_9_mode.coins.append(coin)
...
class Attack:
    ...
    def draw(self):
        if self.mob.mob_type == 'mace': self.mob.attack_image.clip_draw(int(self.mob.frame) * 192, 192 * self.mob.face_dir, 192, 192, self.mob.x, self.mob.y, 300, 300)
        else: self.mob.attack_image.clip_draw(int(self.mob.frame) * 64, 64 * self.mob.face_dir, 64, 64, self.mob.x, self.mob.y, 100, 100)
...
class Move:
    ...
    def draw(self):
        ...
        if self.mob.mob_type == 'bow': self.mob.move_image.clip_draw(int(self.mob.frame) * 128, 128 * self.mob.face_dir, 128, 128, self.mob.x, self.mob.y, 200, 200)
...
class Goblin_Mob:
    def __init__(self):
        ...
        self.move_image = load_image("./image/mobs/goblin/" + self.mob_type + "/walk.png")
        self.idle_image = load_image("./image/mobs/goblin/" + self.mob_type + "/idle.png")
    ...
    def handle_collision(self, group, other):
        ...
        if (group == 'player_sword:goblin_mob' and self.is_alive) or (group == 'player_arrow:goblin_mob' and self.is_alive):
