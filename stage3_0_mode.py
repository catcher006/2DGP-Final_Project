...
def pause():
    global goblin_mobs, stage3_0, coins
    ...
    for goblin_mob in goblin_mobs:
        if goblin_mob.is_alive:
            ...
            stage3_0.saved_mobs.append(mob_data)
            print(f"Pause: Saved mob at ({mob_data['x']}, {mob_data['y']}) with type '{mob_data['type']}', HP: {mob_data['hp']}")

    # 코인 저장
    stage3_0.saved_coins = []
    for coin in coins:
        stage3_0.saved_coins.append({
            'x': coin.x,
            'y': coin.y,
            'frame': coin.frame
        })

    print(f"Pause: Saved {len(stage3_0.saved_mobs)} zombie mobs, {len(stage3_0.saved_coins)} coins")
...
def resume(player_start_pos=None):
    global goblin_mobs, stage3_0, player, coins
    ...
    # 코인 복원
    if stage3_0.saved_coins:
        coins = []
        for coin_data in stage3_0.saved_coins:
